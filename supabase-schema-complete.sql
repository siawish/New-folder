...
-- Created for integration with the Admin Dashboard
...
-- Allow admin to view and edit all profiles
...
-- Allow admin and receptionist to manage all appointments
DROP POLICY IF EXISTS "Admin and receptionist can manage appointments" ON public.appointments;
CREATE POLICY "Admin and receptionist can manage appointments" ON public.appointments
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE INDEX IF NOT EXISTS idx_departments_head_doctor_id ON public.departments(head_doctor_id);
...
-- Insert departments
INSERT INTO public.departments (name, description, location) VALUES
('Cardiology', 'Heart and cardiovascular system', 'Building A, Floor 2'),
('Neurology', 'Brain and nervous system', 'Building B, Floor 1'),
('Pediatrics', 'Medical care for children', 'Building C, Floor 1'),
('Orthopedics', 'Musculoskeletal system', 'Building A, Floor 3'),
('Dermatology', 'Skin related issues', 'Building B, Floor 2');
...
('Insurance', 'Payment through insurance provider'),
('Bank Transfer', 'Direct bank transfer');

-- Insert service catalog
INSERT INTO public.service_catalog (service_code, name, category, description, base_price) VALUES
('CONS-GEN', 'General Consultation', 'Consultation', 'Basic consultation with a general physician', 50.00),
('CONS-SPEC', 'Specialist Consultation', 'Consultation', 'Consultation with a specialist', 100.00),
('LAB-CBC', 'Complete Blood Count', 'Laboratory', 'Blood test to evaluate overall health', 25.00),
('LAB-GLUC', 'Blood Glucose Test', 'Laboratory', 'Test to measure blood sugar levels', 15.00),
('XRAY-CHEST', 'Chest X-Ray', 'Radiology', 'X-ray imaging of the chest', 75.00);

-- Insert tax rates
INSERT INTO public.tax_rates (name, rate, description) VALUES
('Standard', 10.00, 'Standard tax rate for most services'),
('Reduced', 5.00, 'Reduced tax rate for essential services'),
('Zero', 0.00, 'Zero tax rate for exempt services');

-- Insert discount types
INSERT INTO public.discount_types (name, description, rate, is_percentage) VALUES
('Senior Citizen', 'Discount for patients over 65 years', 10.00, true),
('Child', 'Discount for patients under 12 years', 15.00, true),
('Employee', 'Discount for hospital employees', 20.00, true),
('Special', 'Special case discount', 0.00, true);
