-- HMS Enhanced Database Schema for Supabase
-- This script creates the necessary tables for the Hospital Management System with improved structure
...
-- Create profiles table to store common user information
...
-- Add trigger to update email verification status when confirmed in auth.users
...
-- Allow receptionists to manage all appointments
DROP POLICY IF EXISTS "Receptionists can manage all appointments" ON public.appointments;
CREATE POLICY "Receptionists can manage all appointments" ON public.appointments
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE profiles.id = auth.uid() 
            AND (profiles.role = 'receptionist' OR profiles.role = 'admin')
        )
    );

-- Create departments table with improved structure
...
-- Create dedicated doctors table with enhanced fields
...
DROP POLICY IF EXISTS "Patients can view doctor information" ON public.doctors;
CREATE POLICY "Patients can view doctor information" ON public.doctors
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE profiles.id = auth.uid() 
...
-- Create dedicated patients table with enhanced fields
...
-- Create dedicated receptionists table with enhanced fields
...
-- Create medical records table with improved structure
...
-- Allow admin to manage all medical records
DROP POLICY IF EXISTS "Admin can manage all medical records" ON public.medical_records;
...
-- Create medications table with improved structure
...
-- Create prescriptions table with improved structure
...
-- Create prescription_items table with improved structure
...
-- We'll add indexes at the end of the schema after all tables are created

-- Create bedrooms table with improved structure
...
-- Create invoice table with improved structure
...
-- Create invoice_items table with improved structure
...
-- Create payments table with improved structure
...
-- Create payment_methods table with improved structure
...
-- Create service_catalog table with improved structure
...
-- Create tax_rates table with improved structure
...
-- Create discount_types table with improved structure
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'invoices' AND column_name = 'patient_id') THEN
        ALTER TABLE public.invoices ADD COLUMN patient_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
    END IF;
END$$;

-- Ensure payments table has necessary columns
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'payments' AND column_name = 'invoice_id') THEN
        ALTER TABLE public.payments ADD COLUMN invoice_id UUID REFERENCES public.invoices(id) ON DELETE CASCADE;
    END IF;
END$$;

-- Ensure invoice_items table has necessary columns
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'invoice_items' AND column_name = 'invoice_id') THEN
        ALTER TABLE public.invoice_items ADD COLUMN invoice_id UUID REFERENCES public.invoices(id) ON DELETE CASCADE;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'invoice_items' AND column_name = 'item_type') THEN
        ALTER TABLE public.invoice_items ADD COLUMN item_type VARCHAR(50);
    END IF;
END$$;

-- Ensure bedrooms table has necessary columns
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'bedrooms' AND column_name = 'room_number') THEN
        ALTER TABLE public.bedrooms ADD COLUMN room_number VARCHAR(20) UNIQUE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'bedrooms' AND column_name = 'status') THEN
        ALTER TABLE public.bedrooms ADD COLUMN status VARCHAR(20) DEFAULT 'available';
    END IF;
END$$;
...
CREATE INDEX IF NOT EXISTS idx_bedrooms_room_number ON public.bedrooms(room_number);
CREATE INDEX IF NOT EXISTS idx_bedrooms_status ON public.bedrooms(status);
...
-- Create function to update invoice status when payment is made
...
-- Add trigger to update invoice when payment is made
DROP TRIGGER IF EXISTS update_invoice_after_payment_trigger ON public.payments;
...
BEGIN
    ...
    INSERT INTO public.profiles (id, email_verified, role)
    VALUES (NEW.id, NEW.email_confirmed_at IS NOT NULL, 'patient');
    
    -- Create a patient record for the new user (default role)
...
BEGIN
    ...
    SELECT COALESCE(MAX(SUBSTRING(invoice_number FROM 7)::INTEGER), 0) + 1 INTO next_number
    FROM public.invoices
    WHERE invoice_number LIKE invoice_prefix || '-' || year_prefix || '-%';
    
    -- Set the invoice number
    NEW.invoice_number := invoice_prefix || '-' || year_prefix || '-' || LPAD(next_number::TEXT, 5, '0');
    
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Add trigger to generate invoice numbers
...
DECLARE
    year_prefix TEXT;
    next_number INTEGER;
    payment_prefix TEXT := 'PMT';
BEGIN
    -- Get current year for prefix
    year_prefix := to_char(CURRENT_DATE, 'YY');
    
    -- Get the next number in sequence for this year
    SELECT COALESCE(MAX(SUBSTRING(payment_number FROM 7)::INTEGER), 0) + 1 INTO next_number
    FROM public.payments
    WHERE payment_number LIKE payment_prefix || '-' || year_prefix || '-%';
    
    -- Set the payment number
    NEW.payment_number := payment_prefix || '-' || year_prefix || '-' || LPAD(next_number::TEXT, 5, '0');
    
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Add trigger to generate payment numbers
...
-- Add safe indexes for frequently queried columns
-- Only adding indexes for columns we know exist in the database
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'appointments' AND column_name = 'patient_id') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_appointments_patient_id ON public.appointments(patient_id)';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'appointments' AND column_name = 'doctor_id') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_appointments_doctor_id ON public.appointments(doctor_id)';
    END IF;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'medical_records' AND column_name = 'patient_id') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_medical_records_patient_id ON public.medical_records(patient_id)';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'medical_records' AND column_name = 'doctor_id') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_medical_records_doctor_id ON public.medical_records(doctor_id)';
    END IF;
END$$;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'prescriptions' AND column_name = 'patient_id') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_prescriptions_patient_id ON public.prescriptions(patient_id)';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'prescriptions' AND column_name = 'doctor_id') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_prescriptions_doctor_id ON public.prescriptions(doctor_id)';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'prescriptions' AND column_name = 'status') THEN
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'prescription_items' AND column_name = 'prescription_id') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_prescription_items_prescription_id ON public.prescription_items(prescription_id)';
    END IF;
END$$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'audit_logs' AND column_name = 'user_id') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_audit_logs_user_id ON public.audit_logs(user_id)';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'audit_logs' AND column_name = 'table_name') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_audit_logs_table_name ON public.audit_logs(table_name)';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'audit_logs' AND column_name = 'created_at') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_audit_logs_created_at ON public.audit_logs(created_at)';
    END IF;
END$$;
