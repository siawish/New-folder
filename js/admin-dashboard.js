...
const handleNavigation = () => {
    ...
    handleNavigation.showSection = (sectionId) => {
        console.log(`Admin dashboard showing section: ${sectionId}`);
    ...
    // Make handleNavigation available globally for rooms-manager.js
...
const fetchDoctorsData = async () => {
    ...
    try {
        ...
        // Fetch doctors from Supabase with department information
        // Only show doctors whose profiles have email_verified = true
        const { data: doctors, error } = await supabaseClient
            .from('doctors')
        ...
        // Filter out doctors whose email is not verified
        const verifiedDoctors = doctors ? doctors.filter(doctor => 
            doctor.profiles && doctor.profiles.email_verified === true
        ) : [];
        ...
        // Display active doctors from Supabase (only those with verified emails)
        verifiedDoctors.forEach(doctor => {
            ...
            // Get department from either the doctor record or the profile record
            const departmentValue = doctor.department || profile.department || '-';
            ...
            tr.innerHTML = `
                ...
                <td class="px-4 py-3">${departmentValue}</td>
                <td class="px-4 py-3">
...
const initDashboard = async () => {
    try {
        ...
        // Initialize Operations and Medical module
        // Add event listeners for operations tabs
        const appointmentsLink = document.querySelector('.sidebar-item[href="#appointments"]');
        if (appointmentsLink) {
            appointmentsLink.addEventListener('click', () => {
                // Use the showContentSection function from operations-medical.js if available
                if (typeof showContentSection === 'function') {
                    showContentSection('appointments');
                } else {
                ...
                // Load appointments data if the function exists
        ...
        const departmentsLink = document.querySelector('.sidebar-item[href="#departments"]');
        if (departmentsLink) {
            departmentsLink.addEventListener('click', () => {
                // Use the correct function to show the departments section
                ...
                // Load departments data if the function exists
        ...
        // DIRECT FIX: Use a completely standalone approach for Rooms/Beds section
        ...
        if (roomsLink) {
            ...
            // Add a direct click handler that doesn't rely on any other functions
            newRoomsLink.addEventListener('click', function(e) {
                e.preventDefault();
                console.log('DIRECT FIX: Rooms link clicked');
                
                // Hide ALL content sections without relying on the contentSections array
                const allContentSections = document.querySelectorAll('[id$="-content"]');
                allContentSections.forEach(section => {
                    section.classList.add('hidden');
                });
                
                // Show rooms content directly
                const roomsContent = document.getElementById('rooms-content');
                if (roomsContent) {
                    roomsContent.classList.remove('hidden');
                    console.log('DIRECT FIX: Rooms content should now be visible');
                            ...
                            document.querySelectorAll('.delete-room-btn').forEach(button => {
                                button.addEventListener('click', function() {
                                    ...
                                    if (confirm('Are you sure you want to delete this room?')) {
                                        ...
                                        // Refresh the rooms data by triggering the click event again
                                        newRoomsLink.click();
                                    }
                                });
                            });
                ...
                newRoomsLink.classList.add('active', 'bg-blue-100', 'text-blue-600');
            });
        }
        
        // Add event listeners for medical tabs
        const medicalRecordsLink = document.querySelector('.sidebar-item[href="#medical-records"]');
        ...
        const prescriptionsLink = document.querySelector('.sidebar-item[href="#prescriptions"]');
        ...
        const medicationsLink = document.querySelector('.sidebar-item[href="#medications"]');
        if (medicationsLink) {
            medicationsLink.addEventListener('click', () => {
        ...
        initDashboard.showDoctorRegistrationModal = () => {
        ...
        // Function to show patient registration modal
        ...
        if (addDoctorBtn) {
            addDoctorBtn.addEventListener('click', () => {
                // Show doctor registration modal
                initDashboard.showDoctorRegistrationModal();
            });
        }
        
        // Add patient button event listener
        const patientRegBtn = document.getElementById('add-patient-btn');
        if (patientRegBtn) {
            patientRegBtn.addEventListener('click', () => {
        ...
        // Function to show receptionist registration modal
        ...
        if (addReceptionistBtn) {
            ...
            addReceptionistBtn.addEventListener('click', () => {
                // Show receptionist registration modal
                initDashboard.showReceptionistRegistrationModal();
            });
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Operations and Medical module if it exists
    if (window.initOperationsMedical) {
        window.initOperationsMedical();
    }
    
    // Add specific event listener for departments section
    const departmentsLink = document.querySelector('a[href="#departments"]');
    if (departmentsLink) {
        console.log('Found departments link, adding event listener');
        departmentsLink.addEventListener('click', (e) => {
            e.preventDefault();
            console.log('Departments link clicked');
            // Show the departments section
            if (typeof handleNavigation.showSection === 'function') {
                handleNavigation.showSection('departments');
            } else {
                // Fallback if showSection function is not available
                const sections = document.querySelectorAll('[id$="-content"]');
                sections.forEach(section => section.classList.add('hidden'));
                const departmentsContent = document.getElementById('departments-content');
                if (departmentsContent) {
        ...
        console.error('Departments link not found in the sidebar');
    }
});
